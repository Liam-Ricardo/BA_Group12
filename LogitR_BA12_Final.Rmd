...
# Create binary target variable using ONLY a subset of the features
df <- df %>%
  mutate(
    weighted_score = (title_score * 20) + (video_tag_score * 20),
    trending = as.factor(ifelse(weighted_score >= quantile(weighted_score, 0.75, na.rm = TRUE), 1, 0))
  ) 
#  select(-title, -daily_rank, -daily_movement, -weekly_movement, -snapshot_date,
        # -view_count, -like_count, -comment_count, -video_tags, -publish_date, -weighted_score)
...
logit_model <- glm(trending ~ engagement_score + comment_score + video_age,
