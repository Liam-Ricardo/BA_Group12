...
```{r group the dataset per country}
...
```{r plotting the data grouped by countries}
...
yt_df <- yt_df %>% select(-country)
```

### 2.2 Language vs. Days to Trend
Objective: To explore whether the language of a video influences how quickly it trends.

Why this matters: Language may reflect the audience size, cultural virality, or regional algorithm differences. A video in a widely spoken or digitally active language might trend faster due to more immediate engagement.

Before performing bivariate analysis, we need to calculate the number of days a video takes to trend after its initial publishing. This is done by subtracting the publish_date from the snapshot_date (date when the video appeared on the trending list).
...
Based on the summary statistics of days_to_trend
The median of 5 days shows that most videos trend within a week.
The long tail (max 38 days) shows that some videos gain attention much later.
The mean (6.57) being higher than the median suggests a slightly right-skewed distribution.
Now, we can actually perform bivariate analysis based o days to trend as well as the language.
...
```
This plot displays the median number of days it takes for a video to trend, grouped by the language it's published in. We sort the bars to better observe which languages tend to trend faster. Languages like Russian (ru), Japanese (ja), and Korean (ko) show higher median values — suggesting videos in these languages tend to trend more slowly. On the contrary, languages with lower medians indicate faster trending, possibly due to higher digital activity or algorithmic priority in those regions.
This analysis can help content creators target certain linguistic audiences if their goal is to trend quickly.

### 2.3 Like/View Ratio vs. Days to Trend
...
```
Here, we calculate the like/view ratio as a proxy for viewer engagement. This ratio is plotted against the number of days it took the video to trend. A linear regression line is also added to observe the overall trend.
There is a noticeable negative correlation: videos with a higher like/view ratio tend to trend faster. This supports the idea that engagement quality matters more than just raw view counts.
The regression line slopes downward, further confirming that better audience feedback (likes) relative to views helps videos gain trending momentum more quickly.

### 2.4 Day of Publish vs. Days to Trend
...
We extract the day of the week from the video’s publish date and examine if it impacts the time taken to trend. Days are ordered from Monday to Sunday for a clean comparison.
Interestingly, the median time to trend remains consistent across all days, around 5 days. This implies that publish timing does not significantly impact how quickly a video trends. It may indicate that content quality and engagement outweigh the influence of timing, at least when viewed from a weekly scale.
...
```{r}
# Update this variable with the your own filepath 
 data_filepath <- "C:/Users/LRJPu/OneDrive - Delft University of Technology/Documenten/TU delft/MOT/Y1-Q3/BA/Assignment/BA_Group12_Final/trending_yt_videos_113_countries_model_training.csv"
...
Samilar as `Random Forest`, we can see that `video_tag_score` is the least important variable compared to `title_score` , `engagement_score` , `comment_score` , and `video_age`.
...
Based on accuracy both the models have higher values, but we need good precision (Postive Predictive Value) and Recall (sensitivity) for our use case, ie reducing the number of errors (both Type 1 and Type 2) to better understand the factors that are influencing the trendiness of a video.

So, based on these conditions we will be using the `Logistic Regression` model to predict the trending videos moving forward. As we can see from the importance of the features, a higher `engagement_score` (ie like to view count), `comment_score` (ie comment to view count), and `title_score` (ie the title with catchy words) can influence the video's trendiness. Other factors like `video_age`, and `video_tag_score` can influence it a bit, but better to focus on gaining more subscribers (to increase the `engagement_score` and `comment_score`) and using catchy words in the Title.
...
1.   **Focus on Boosting Engagement:**
    -   Prioritize increasing likes and comments, as these are the strongest predictors of trendiness.
    -   Tactics to consider include launching videos alongside giveaways, exclusive offers, or campaigns that encourage users to like and comment.
...
3.   **Refine Use of Tags (Optional):**
    -   Consider tags as an additional optimization lever after engagement and titles are addressed.
    -   While tags are less influential, you can still use the `final_tags_words` list to inform your selection of tags.
