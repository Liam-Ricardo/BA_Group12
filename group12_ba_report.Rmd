...
As we can see from the confusion matrix, the Random Forest model has pretty good `accuracy` `(~ 99.48%)` - *sounds like an overfitted model* - But this is okay, since the count of positive class is very low compared to the negative class. We have used the cross-validation and hyperparameter tuning while training the model - this should suppress the overfitting problem. Moving on to the other scores, we have got a pretty good `precision (63.82%)` - meaning that our model is correctly classifying the trending videos as trending. This is something which we really want. The `Recall` and `F1 scores (~10.05% & ~17.26%)` are in the lower range. This could be improved using weights for the classes.
