---
title: "Group 12 - Report - BA Assignment"
...
packages <- c("dplyr","tidyr","stringr","lubridate","ggplot2","textTinyR","tm","hunspell","Snoballc","factoextra","gmodels","c50","Hmisc","rsample","caret","tidyverse","countrycode","VennDiagram")
...
clean_data <- function(df){      
  ...
  df <- df %>% select(-c(channel_name, description, thumbnail_url, kind, video_id, channel_id, langauge))
...
```{r cleaning_unused_memory1, include=FALSE}
gc()
```
...
```{r cleaning_unused_memory2, include=FALSE}
gc()
```
...
```{r cleaning_unused_memory3, include=FALSE}
# This block removes all the elements from your environment and cleans up unused memory from the session
vars_list = c("yt_df","data_filepath")
remove(list=setdiff(ls(), vars_list))
gc()
```
...
analyze_numerical <- function(df) {
  ...
  for (col in numerical_cols) {
    ...
    pic_2 <- ggplot(df, aes(y = .data[[col]])) + 
...
For a more detailed overview of analysis per numerical variable and corresponding plot, please see the table below.
...
```{r cleaning_unused_memory4, include=FALSE}
gc()
```
...
The variable `language` had a lot of empty values, and different type of values e.g.: "zh-TW", or "ja",or "en-US". There are even 2280084 values "missing" which are instances in which the language of the video was not recognized. This is too much so we decided to not include this variable.

### 2. Bivariate Analysis

#### 2.1. Country and video trendiness

Let us take a look at the `country` column, as it may yield interesting geographical insights into the trendiness of a video. Initially, this column uses ISO2 country codes, which are not very readable. For clarity, we generate a new column that maps these codes to their full country names.

```{r full_country_names}
yt_df$full_country_name <- countrycode(
  sourcevar = yt_df$country,
  origin = "iso2c",
  destination = "country.name",
  warn = FALSE
)

unique_countries <- unique(yt_df$full_country_name)
print(unique_countries[0:5])
summary(unique_countries)
```

After creating this new `full_country_name` column, we observe that the dataset includes videos from 113 distinct countries. This diversity provides a good opportunity to analyze country-level differences in video trendiness.

To begin, we group the dataset by country and count the number of videos per country to understand how frequently each appears in the data.

```{r group_dataset_per_country}
grouped_df <- yt_df%>% group_by(country)
...
This reveals a small but clear difference in video frequency across countries - with a `minimum` of `24074` videos and a `maximum` of `24359`. This distribution could reflect differences in regional usage of YouTube videos.

To delve deeper, we calculate the median daily rank for each country. Since for now a lower daily rank indicates higher trendiness, this measure helps us identify which countries tend to produce trendier content. To maintain clarity and focus on meaningful insights, we narrow our analysis to the top 10% of countries with the lowest median daily rank.

```{r plot_grouped_data_by_countries}
# 1. Calculate the median daily_rank and total video count for each country
country_summary <- yt_df %>%
  group_by(country) %>%
  summarise(
...
This boxplot visualization of the top 10% of countries confirms that daily_rank is calculated within each country, not globally. Given that the daily rank ranges from 0 to 50 within countries, it's intuitive that the median sits around 25. This supports the common industry understanding that YouTube trends are often localized, with algorithms tailoring recommendations based on the viewer's region.

Next, we take a broader view by identifying the top 10% of countrie**s** based on three different performance metrics:

1.  Average view count
...
```{r}
num_top_countries <- round(0.10 * n_distinct(yt_df$country))

# 1. Top 10% countries by highest average view count
top_avg_view_count <- yt_df %>%
  group_by(country) %>%
...
These lists help us explore whether the same countries consistently dominate across multiple measures of success. For example, a country that appears in all three lists could be a strategic focus area for content promotion.

To visualize the overlap between these three top-performing groups, we generate a Venn diagram.

```{r VennDiagram_3_lists}
...
It shows that out of the 11 countries in each list, 6 countries appear in both the top 10% based on average view count and average like count. However, it is interesting that no single country appears in the top 10% across all three performance metrics: average views, average likes, and total video count. This suggests that different countries excel in different aspects of video trendiness.

While the geographical analysis provided valuable insights—highlighting regional variations in daily rank and engagement patterns—we have decided not to include the `country` column in the remainder of the report. This decision is based on the assumption that our target audience, marketing industry professionals, are aiming for global reach. Therefore, their primary interest lies in identifying the overall drivers of trendiness on YouTube, regardless of geographical location. From this point forward, the focus will shift to universal predictors of video success across all countries.

```{r dropping the country column}
yt_df <- yt_df %>% select(-c("country", "full_country_name"))
```

#### 2.2. Like/View Ratio vs. Days to Trend

Objective: To analyze how the like-to-view ratio affects the time it takes for a video to trend.

Why this matters: Engagement is a strong signal for virality. Videos with high like/view ratios may be seen as higher quality or more engaging, possibly accelerating their rise to trending.
...
Here, we calculate the like/view ratio as a proxy for viewer engagement. This ratio is plotted against the number of days it took the video to trend. A linear regression line is also added to observe the overall trend. There is a noticeable negative correlation: videos with a higher like/view ratio tend to trend faster. This supports the idea that engagement quality matters more than just raw view counts. The regression line slopes downward, further confirming that better audience feedback (likes) relative to views helps videos gain trending momentum more quickly.

#### 2.3. Day of Publish vs. Days to Trend

Objective: To identify whether publishing on specific days of the week affects how soon a video trends.

Why this matters: YouTube user traffic may vary by day — for example, weekends might have higher user activity. Knowing this can help with timing strategy for releasing videos.
...
We extract the day of the week from the video’s publish date and examine if it impacts the time taken to trend. Days are ordered from Monday to Sunday for a clean comparison. Interestingly, the median time to trend remains consistent across all days, around 5 days. This implies that publish timing does not significantly impact how quickly a video trends. It may indicate that content quality and engagement outweigh the influence of timing, at least when viewed from a weekly scale.

```{r cleaning_unused_memory5, include=FALSE}
# This block removes all the elements from your environment and cleans up unused memory from the session
vars_list = c("yt_df","final_tag_words","final_title_words","final_results","tokenize","stopwords_list","data_filepath")
remove(list=setdiff(ls(), vars_list))
gc()
```
...
```{r cleaning_unused_memory6, include=FALSE}
# This block removes all the elements from your environment and cleans up unused memory from the session
vars_list = c("yt_df","final_tag_words","final_title_words","final_results","tokenize","stopwords_list","data_filepath")
remove(list=setdiff(ls(), vars_list))
gc()
...
```{r export_model_training, include=FALSE}
#export_data <- function(df, input_filepath){   
...
```{r cleaning_unused_memory7, include=FALSE}
# This block removes all the elements from your environment and cleans up unused memory from the session
vars_list = c("yt_df","data_filepath")
remove(list=setdiff(ls(), vars_list))
gc()
...
```{r}
# Update this variable with the your own filepath 
# data_filepath <- "./datasets/trending_yt_videos_113_countries_model_training.csv"
...
# setting the default seed to train a reproducable model
set.seed(123)
...
# splitting the data to be used for final validation
...
> *Note - We have commented this piece of code as it was throwing error while knitting. Since, we couldn't run everything from a single notebook (or) with different notebooks in a single run, we have separated the model training section to a different notebook. But we have added screenshots from that run in-place of the actual output.*
...
# random forest model - commenting for knitting purposes
# rf_model <- train(
#  formula,
#  data = train_data,
#  method = "rf",
...
Now, that we have our model trained with the training data (including cross-validation and hyperparameter tuning). Let's go ahead and check the model parameters. We will also have an option to export the model into a `.RData` file, so that it can be used in the future.
...
![Random Forest - Feature Importance](rf_model_images/rf_importance.jpg)
...
![Random Forest - Confusion Matrix](rf_model_images/rf_confusion_matrix.jpg)

As we can see from the confusion matrix, the Random Forest model has pretty high `accuracy (~ 99.48%)` - *sounds like an overfitted model* - But this is okay, since the count of positive class is very low compared to the negative class. We have used the cross-validation and hyperparameter tuning while training the model. This should suppress the overfitting problem. Moving on to the other scores, we have got a pretty good `precision (63.82%)` meaning that our model is correctly classifying the trending videos as trending. This is something which we really want. The `Recall` and `F1 scores (~10.05% & ~17.26%)` are in the lower range which could be improved using weights for the classes.

### **2. Logistic Regression**

We will be repeating the `steps 1.2 to 1.4` for Logistic Regression.

> *Note - When running for the first time - make sure to enter "yes" in the terminal - to install the required packages*
...
Same as `Random Forest`, we can see that `video_tag_score` is the least important variable than the others in predicting the video's trendiness.
...
Based on accuracy, both the models have higher values, but we need good precision (Postive Predictive Value) and Recall (sensitivity) for our use case (i.e. reducing the number of errors (both Type 1 and Type 2) to better understand the factors that are influencing the trendiness of a video).

Based on these conditions, we will be using the `Logistic Regression` model to predict the trending videos moving forward. As we can see from the importance of the features, a higher `engagement_score` (i.e. like-to-view count), `comment_score` (i.e. comment-to-view count), and `title_score` (i.e. the title with catchy words) can influence the video's trendiness. Other factors such as `video_age` and `video_tag_score` can influence it somewhat but better to focus on gaining more subscribers (to increase the `engagement_score` and `comment_score`) and use catchy words in the `title`. Doing so will keep the users engaged and it will make your video to come out on top of the trending list.

```{r cleaning_unused_memory8, include=FALSE}
# This block removes all the elements from your environment and cleans up unused memory from the session
remove(list=ls())
gc()
```
...
In conclusion, the `Logistic Regression` model is the most suitable for addressing our business problem: predicting whether a YouTube video will become trendy. While both models achieved high accuracy, Logistic Regression demonstrated better balance in precision and recall, which is essential for minimizing both false positives and false negatives.

Our analysis confirms that *audience engagement* is the most critical factor influencing a video's trendiness. Specifically, the like-to-view and comment-to-view ratios emerged as strong predictors. These metrics reflect the strength of user interaction and should therefore be prioritized when evaluating or planning video content.

In addition, the title of the video plays a important role in capturing initial attention. Videos that contain certain high-performing keywords (as defined in our generated list `final_title_words`) have a higher likelihood of trending.

Interestingly, our model challenges two commonly held industry assumptions:

-   *Time-sensitivity* (e.g., going viral within 24–72 hours) appears to be less influential than engagement metrics.

-   *Video tags,* although often emphasized in optimization strategies, show limited predictive value for trendiness compared to likes, comments, and title wording.

### Recommendations

To improve the likelihood that a video will trend, we recommend the following strategies:

1.  **Focus on Boosting Engagement:** - Prioritize increasing likes and comments, as these are the strongest predictors of trendiness. - Tactics to consider include launching videos alongside giveaways, exclusive offers, or campaigns that encourage users to like and comment.
2.  **Optimize Video Titles:**
    -   Place strong emphasis on crafting an effective title, as it is the second most influential factor after engagement.

    -   Incorporate as many relevant terms as possible from the `final_title_words` list into your video titles as they were identified as having the highest positive impact on trendiness in our model.
3.  **Refine Use of Tags (Optional):** - Consider tags as an additional optimization lever after engagement and titles are addressed. - While tags are less influential, you can still use the `final_tags_words` list to inform your selection of tags.
