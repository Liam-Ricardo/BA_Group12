...
We have now completed our EDA and understand the dataset completely. We can create features (or update the existing columns) that will be fed as inputs to the model. There are various methods of feature engineering such as selecting the just important columns or creating new columns by merging existing columns.

### 1. Convert Text Columns into Numerical Columns

First, we start by converting the text columns (`title` and `video_tags`, in our case) into numerical columns, for better model predictability. Then, we introduce the `scores` of the columns using the following logic (`title_score` and `video_tags_score` columns): take the value in a cell—say the 1st cell of the `title` column: "*Highlights Real Madrid vs Barcelona 3-1...*" Run the words of the title separately through the list of words in `final_title_words`. Increase the count by 1 every time the word in the cell matches a word in the list. Example: the title "*Highlights Real Madrid vs Barcelona 3-1..."* will get a score of **3.**
...
This function has added a column to track the counts. We will be using the synthetic columns in place of the original text columns while training the model.
...
Let's prepare the data to be fed into the model. We will be creating a few additional columns such as `engagement_score`, `comment_score`, `video_age`, and the target column itself: `trending`. The formula used for the columns are as follows:
...
-   `comment_score` = `comment_count` / `view_count` (Out of the total audience (people who view the video) how many people are commenting on the video?)
...
To create the target variable (`trending`), we start by calculating the `trend_score`. In simple terms, the video that gets a `trend_score` of more than 80 (out of 100) will be considered as the positive class (`trending = 1`). The logic to calculate `trend_score` is as follows:
...
Now that we have prepared our dataset (under the name `yt_df`), we can go ahead and select the appropriate models to classify whether a video will become trendy. Since this is a binary classification problem (trendy vs not trendy), we selected models well-suited for this task.

First, we chose RandomForest. This ensemble method is known for its high performance on classification problems due to its ability to handle non-linear relationships, reduce overfitting through bagging, and provide insights via feature importance. It is also relative robust to noise in the data. We briefly considered Gradient Boosting, which builds models sequentially to correct errors from previous models. However, given our current goals of establishing an interpretable baseline, we decided to start with RandomForest due to its simplicity and faster training time

Second, we selected Logistic Regression. Widely used as a baseline for binary classification, it offers strong interpretability. Logistic Regression is similar to Linear Regression but is instead used with a categorical response (trendy or not trendy).
...
In this section, we will be further prepping the dataset to be fed into the model. Namely, we will be defining the `feature` and `target` variables, and `train-validation` split.
...
<<<<<<< HEAD
#### 1.3. Exporting the model (Optional)
=======
#### 1.3. Exporting the Model
>>>>>>> 3865b9379c9bcd8b0769f7a9ee6a23f590ad3905
...
<!--# Using class weights can go in our further steps/limitations section -->

<!--# Also, when we re-run we might have to re-calculate these scores -->
...
<!--# Explanation of the scores needs to be added -->

Based on accuracy, both the models have higher values, but we need good precision (Pos Predictive Value) and Recall (sensitivity) for our use case (i.e. reducing the number of errors (both Type 1 and Type 2) to better understand the factors that are influencing the trendiness of a video).
