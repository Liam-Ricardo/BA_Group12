...
To summarize, the best model suited for our business problem – whether a YouTube video would become trendy or not is the `Logistic Regression` model. For industry experts involved in setting up marketing campaigns it is good to noticed that the interaction of an audience with your content is the main driver for trendiness. The ratios that are best tracked of a single video are like to view and comment to view count. This is in line with what is already commonly seen as a best practice to focus on.

Next to this the title needs to contain the right words to increase likelihood of trendiness of a video. Based on the current dataset we have defined a list called `final_title_words` that can be used to see which words to include in the title.

and `final_tags_words`

In contrast to some best practices, in which it is believed that trendiness of a video has to be reached within a certain time frame e.g. 24-72 hours, our results suggest that this is of little influence compared to the engagement metrics. Another over stressed best practice is to make sure you use much as possible and the correct tags for a video, this has also been shown by the model evaluation to not be as important compared to engagement metrics.

### Recommendations

Based on the results we have the following recommendations for industry professionals to increase the likelihood of a video to reach trendiness:

1.  Prioritize engagement metrics such as likes and comments over tags and video age.
2.  Use as many words as possible from the generated list called `final_title_words` to include in your video's title.
3.  To further optimize use the generated list called `final_tags_words` to ensure best tags for trendiness.
