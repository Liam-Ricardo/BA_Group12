...
Our criteria for being a "desirable" outlier is as follows:
...
Since criteria #3-6 are based on existing variables in the dataset, let's start by establishing the ratios for criteria #1 and #2.
...
Because snapshot_date is the same for all videos, we can drop that column. We will also drop video_tags and that will be explored in a separate section.
