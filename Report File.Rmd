...
```
```{r}
ggplot(df_filtered, aes(x = view_count, y = daily_rank, color = country)) +
  geom_point() +
  facet_wrap(~ country) +
...
df$primary_language_numeric <- as.numeric(factor(df$primary_language))
df$channel_name_numeric <- as.numeric(factor(df$channel_name))

#re-do correlation analysis
cor_matrix <- df %>%
...
Let's continue with some data preparation the column "langauge" why does it seem to have 1)empty values, 2)different type of values e.g.: "zh-TW", or "ja",or "en-US"
```{r}
print(df$langauge[0:5])
```
```{r}
# Remove leading/trailing whitespace
df$langauge <- trimws(df$langauge)

# Replace empty strings with NA
df$langauge[df$langauge == ""] <- NA

# Extract the primary language code
df$primary_language <- sub("-.*", "", df$langauge)

#creating a new column for regional variants (if needed)
df$region <- sub("^[^-]*-?", "", df$langauge)
