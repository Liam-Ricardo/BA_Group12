...
# cleaned_df <- read.csv(data_filepath, stringsAsFactors = FALSE)
...
correct_spelling <- function(words) {
  corrected_words <- unlist(sapply(words, function(word) {
    suggestions <- hunspell_suggest(word)
    if (length(suggestions) > 0) {
      return(suggestions[[1]])  # Pick the first suggestion
    } else {
      return(word)  # Keep original word if no suggestion available
    }
  }))
  return(sort(unique(corrected_words)))
}
